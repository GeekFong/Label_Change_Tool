...
## **简介**
这款工具是一个标签转换工具。在yolo中完成初始模型的训练后，实际运行环境中常常出现误识别的情况。为了解决这个问题，我开始收集这些被误识别的图片。接下来，我使用初始模型重新识别这些图片，生成了包含误识别信息的txt标注文件。但是标注精灵需要的是xml格式的标注文件，所以需要进行转换，以便在标注精灵中重新标注那些出现误识别的地方。
