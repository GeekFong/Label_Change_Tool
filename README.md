...
## **简介**
创做该工具的初衷, 是因为在用yolo在做识别，训练出初始模型后，在实际环境中运行会出现误识别。然后把误识别图片采集起来。再使用模型把误识别的图片用初始模型识别后得出txt的标注文件，再把txt标注文件放到标注精灵中把误识别地方进行重新标注，从而减少标注的时间。最后还需要转会yolo的txt标注格式再次训练新的模型。这些转换步骤非常繁琐，容易出错，因此特意制作出这款转换工具并且分享出来，希望能够帮助到和我有相同需求的人。
